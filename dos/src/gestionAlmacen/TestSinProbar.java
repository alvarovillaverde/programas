...
//	    }
	...
	public static void cargarColeccionDesdeAlmacenamiento() {
		...
		try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fichero))) {
			...
			if (objetoLeido instanceof ArrayList) {
				ArrayList<Articulo> listaCargada = (ArrayList<Articulo>) objetoLeido;
				
				// 3. Importante: Pasamos la lista al objeto 'almacen' que ya creamos
    ...
    try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("almacen.obj"))) {
        ...
        // Obtenemos la lista a través del getter
        ArrayList<Articulo> lista = almacen.getListado();
        
        // Ordenamos antes de guardar (Asegúrate que Articulo implemente Comparable)
        Collections.sort(lista);
        
        oos.writeObject(lista);
        System.out.println("Colección guardada correctamente en almacen.obj");
